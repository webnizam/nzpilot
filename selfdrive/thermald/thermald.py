...
def thermald_thread():
    ...
    pandaState_sock = messaging.sub_sock(
        'pandaStates', timeout=pandaState_timeout)
    ...
    controller = PIDController(
        k_p=0, k_i=2e-3, neg_limit=-80, pos_limit=0, rate=(1 / DT_TRML))
    ...
    while True:
        ...
        max_comp_temp = temp_filter.update(
            max(max(msg.deviceState.cpuTempC),
                msg.deviceState.memoryTempC, max(msg.deviceState.gpuTempC))
        )

        if handle_fan is not None:
            fan_speed = handle_fan(
                controller, max_comp_temp, fan_speed, onroad_conditions["ignition"])
        ...
        if TICI:
            set_offroad_alert_if_changed(
                "Offroad_StorageMissing", (not Path("/data/media").is_mount()))
        ...
        if not os.path.isdir("/data/openpilot"):
            if is_openpilot_dir:
                # Delete DongleID if the Openpilot directory disappears, Seems you want to switch fork/branch.
                os.system("cd /data/params/d; rm -f DongleId")
        ...
        ui_running = "ui" in (
            p.name for p in sm["managerState"].processes if p.running)
        ...
        if (count % int(600. / DT_TRML)) == 0:
            ...
            cloudlog.event("STATUS_PACKET",
                           count=count,
                           pandaStates=(strip_deprecated_keys(
                               pandaStates.to_dict()) if pandaStates else None),
                           peripheralState=strip_deprecated_keys(
                               peripheralState.to_dict()),
                           location=(strip_deprecated_keys(sm["gpsLocationExternal"].to_dict(
