...
class opEdit:  # use by running `python /data/openpilot/op_edit.py`
    ...
    def run_init(self):
        if self.username is None:
            self.success('\nWelcome to the {}opParams{} command line editor!'.format(
                COLORS.CYAN, COLORS.SUCCESS), sleep_time=0)
            self.prompt(
                'Would you like to add your Discord username for easier crash debugging for the fork owner?')
            self.prompt(
                'Your username is only used for reaching out if a crash occurs.')

            username_choice = self.input_with_options(
                ['Y', 'N', 'don\'t ask again'], default='n')[0]
            ...
            self.success('\nWelcome to the {}opParams{} command line editor, {}!'.format(
                COLORS.CYAN, COLORS.SUCCESS, self.username), sleep_time=0)
    ...
    def run_loop(self):
        while True:
            if not self.live_tuning:
                ...
                self.info('(non-static params update while driving)',
                          end='\n', sleep_time=0)
            else:
                self.info('Here are your live parameters:', sleep_time=0)
                self.info('(changes take effect within a second)',
                          end='\n', sleep_time=0)
            ...
            if self.live_tuning:  # only display live tunable params
                ...
                ) if self.op_params.fork_params[k].live}
            ...
            static = [
                COLORS.INFO + '(static)' + COLORS.ENDC if self.op_params.fork_params[k].static else '' for k in self.params]

            to_print = []
            blue_gradient = [33, 39, 45, 51, 87]
            for idx, param in enumerate(self.params):
                line = '{}. {}: {}  {}'.format(
                    idx + 1, param, values_list[idx], static[idx])
                if idx == self.last_choice and self.last_choice is not None:
                    line = COLORS.OKGREEN + line
                else:
                    _color = blue_gradient[min(
                        round(idx / len(self.params) * len(blue_gradient)), len(blue_gradient) - 1)]
            ...
            to_print += ['---'] + ['{}. {}'.format(ext_col + e, ext_txt + COLORS.ENDC)
                                   for e, (ext_txt, ext_col) in extras.items()]
            print('\n'.join(to_print))
            self.prompt('\nChoose a parameter to edit (by index or name):')

            choice = input('>> ').strip().lower()
            parsed, choice = self.parse_choice(
                choice, len(to_print) - len(extras))
            if parsed == 'continue':
                continue
            elif parsed == 'change':
    ...
    def parse_choice(self, choice, opt_len):
        ...
        if choice in ['l', 'live']:  # live tuning mode
            ...
            param_sims = [(idx, self.str_sim(choice, param.lower()))
                          for idx, param in enumerate(self.params)]
            param_sims = [param for param in param_sims if param[1] > 0.33]
            if len(param_sims) > 0:
                chosen_param = sorted(
                    param_sims, key=lambda param: param[1], reverse=True)[0]
        ...
        while True:
            ...
            if not param_info.static:
                self.info2('Chosen parameter: {}{} (live!)'.format(
                    chosen_key, COLORS.BASE(207)), sleep_time=0)
            else:
            ...
            if param_info.static:
                to_print.append(
                    COLORS.WARNING + '>>  A reboot is required for changes to this parameter!' + COLORS.ENDC)
            if not param_info.static and not param_info.live:
                to_print.append(
                    COLORS.WARNING + '>>  Changes take effect within 10 seconds for this parameter!' + COLORS.ENDC)
            if param_info.has_allowed_types:
                to_print.append(COLORS.RED + '>>  Allowed types: {}'.format(
                    ', '.join([at.__name__ for at in param_info.allowed_types])) + COLORS.ENDC)
            ...
            if param_info.is_list:
                ...
                self.change_param_list(old_value, param_info, chosen_key)
                return

            self.info('Current value: {}{} (type: {})'.format(self.color_from_type(
                old_value), COLORS.INFO, type(old_value).__name__), sleep_time=0)
            ...
            while True:
                ...
                if not param_info.is_valid(new_value):
                    self.error('The type of data you entered ({}) is not allowed with this parameter!'.format(
                        type(new_value).__name__))
                    continue

                if not param_info.static:  # stay in live tuning interface
                    self.op_params.put(chosen_key, new_value)
                    self.success('Saved {} with value: {}{}! (type: {})'.format(
                        chosen_key, self.color_from_type(new_value), COLORS.SUCCESS, type(new_value).__name__))
                else:  # else ask to save and break
                    self.warning('\nOld value: {}{} (type: {})'.format(
                        self.color_from_type(old_value), COLORS.WARNING, type(old_value).__name__))
                    self.success('New value: {}{} (type: {})'.format(self.color_from_type(
                        new_value), COLORS.OKGREEN, type(new_value).__name__), sleep_time=0)
    ...
    def change_param_list(self, old_value, param_info, chosen_key):
        while True:
            self.info('Current value: {} (type: {})'.format(
                old_value, type(old_value).__name__), sleep_time=0)
            self.prompt('\nEnter index to edit (0 to {}):'.format(
                len(old_value) - 1))
            ...
            while True:
                ...
                self.info('Value: {} (type: {})'.format(old_value[choice_idx], type(
                    old_value[choice_idx]).__name__), sleep_time=0)
                ...
                self.success('Saved {} with value: {}{}! (type: {})'.format(chosen_key, self.color_from_type(
                    new_value), COLORS.SUCCESS, type(new_value).__name__), end='\n')
                break
    ...
    def str_color(msg, style, surround=False):
        ...
        if surround:
            msg = '{}--------\n{}\n{}--------{}'.format(
                style, msg, COLORS.ENDC + style, COLORS.ENDC)
        else:
